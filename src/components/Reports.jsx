...
import { getReports } from '../context/services/client';
    ...
    return (
        <div className='flex flex-col gap-[2rem] mx-[1rem]'>
            ...
            <div className='bg-white rounded-[1rem] w-full'>
                ...
                <ReportCard label="Analytics & Reporting" darkHeading={true} value={`All Users: ${reports.customers_by_year.reduce((a, b) => a + b, 0)}`} filterOptions={["Today"]} onClick={() => { setShowCustomers(!showCustomers); setShowVendors(!showVendors); }}>
                    <LineChart width={1000} height={300} data={data}>
                        ...
                        <Tooltip />
                        <Legend />
                        {showCustomers && <Line type="monotone" dataKey="Students" stroke="#BCE8B1" strokeWidth={4} />}
                        {showVendors && <Line type="monotone" dataKey="Educators" stroke="#A8B6FF" strokeWidth={4} />}
                    </LineChart>
                </ReportCard>
            </div>
